...
describe('command-line.helper', () => {
    ...
    describe('normaliseConfig', () => {
        it('should replace type constructors with objects', () => {
            const normalised = (0, command_line_helper_1.normaliseConfig)(getConfig());
    ...
    describe('createCommandLineConfig', () => {
        it('should create expected config', () => {
            const commandLineConfig = (0, command_line_helper_1.createCommandLineConfig)((0, command_line_helper_1.normaliseConfig)(getConfig()));
    ...
    describe('mergeConfig', () => {
        ...
        fileConfigTests.forEach((test) => {
            it(`should return configFromFile when ${test.description}`, () => {
                const result = (0, command_line_helper_1.mergeConfig)(test.parsedArgs, test.parsedArgsNoDefaults || test.parsedArgs, test.fileContent, options, test.jsonPath);
        ...
        typeConversionTests.forEach((test) => {
            it(`should convert all configfromFile properties with type conversion function with input: '${JSON.stringify(test.fromFile)}'`, () => {
                const result = (0, command_line_helper_1.mergeConfig)({}, {}, test.fromFile, options, undefined);
                expect(result).toEqual(test.expected);
            });
        });
    });
